...
# ── Config ────────────────────────────────────────────────────────────────────
app          = FastAPI()
OLLAMA_URL   = "http://192.168.0.6:11434"   # Ajusta a la IP de tu servidor Ollama si es externo
DIR_PROMPTS  = "prompts"
...
def init_db():
    ...
    # Migraciones para columnas nuevas en DBs antiguas
    migraciones = [
...
def extraer_meta(model_id: str) -> Dict[str, Any]:
    meta = {"cuantizacion":"—","tamano":"—","familia":"—","contexto_max":4096,"size_gb":0}
...
async def ollama_estado():
    async with httpx.AsyncClient(timeout=3) as client:
        try:
            ...
            modelos_activos = resp.json().get("models", [])
            info = [{"modelo": m.get("name","")} for m in modelos_activos]
            return {"ocupado": len(modelos_activos) > 0, "modelos": info}
        except Exception:
...
async def inferencia(payload: InferenciaPayload, request: Request):
    ...
    ollama_payload = {
        "model": payload.modelo, "messages": mensajes, "stream": True,
        "options": { "temperature": payload.temperatura, "top_p": payload.top_p, "top_k": payload.top_k, "num_predict": payload.max_tokens }
    }
    ...
    async def stream_gen():
        start, prompt_tokens, output_tokens, prompt_dur_ns, eval_dur_ns, got_prompt = time.time(), 0, 0, 0, 0, False
        try:
            async with httpx.AsyncClient(timeout=timeout) as client:
                async with client.stream("POST", f"{OLLAMA_URL}/api/chat", json=ollama_payload) as resp:
                    async for line in resp.aiter_lines():
                        ...
                        prompt_dur_ns = data.get("prompt_eval_duration", prompt_dur_ns)
                        output_tokens = data.get("eval_count", output_tokens)
                        eval_dur_ns   = data.get("eval_duration", eval_dur_ns)

                        if not got_prompt and prompt_tokens > 0:
                            got_prompt = True
                            delta_tok  = max(0, prompt_tokens - payload.prompt_tokens_prev)
                            pt_tps     = round(prompt_tokens / (prompt_dur_ns/1e9), 1) if prompt_dur_ns > 0 else 0
                            yield f"[PROMPT_DONE]||{prompt_tokens}||{pt_tps}||{delta_tok}\n"
                        ...
                        if data.get("done", False):
                            ...
                            ratio = round(prompt_dur_ns / eval_dur_ns, 3) if eval_dur_ns > 0 else 0
                            delta_tok = max(0, prompt_tokens - payload.prompt_tokens_prev)
                            yield f"[DONE]||{total}||{tps_final}||{prompt_tokens}||{output_tokens}||{delta_tok}||{ratio}\n"
                            return
...
async def exportar_guion(id_sesion: str):
    ...
    # Python
    script  = f'"""Guion de replicacion — {titulo}\\nGenerado por LAB21 v3.2"""\n\nimport httpx, json\n\n'
    script += f'OLLAMA_URL = "{OLLAMA_URL}"\nMODELO = "{modelo}"\nSYSTEM_PROMPT = """{sys_p}"""\n\n'
    script += f'PARAMS = {json.dumps({"temperature": params.get("temperatura",0.8), "top_p": params.get("top_p",0.95), "top_k": params.get("top_k",40), "num_predict": params.get("max_tokens",1024)}, indent=4)}\n\n'
    script += 'PROMPTS = [\n'
    ...
    for i, prompt in enumerate(PROMPTS):
        ...
        resp = httpx.post(f"{OLLAMA_URL}/api/chat", json={"model": MODELO, "messages": mensajes, "stream": False, "options": PARAMS}, timeout=300)
...
# ── Finalizar (Actualización y Titulación) ────────────────────────────────────
    ...
    if titulo_ia.startswith("Sesión ") or not titulo_ia:
        ...
        try:
            async with httpx.AsyncClient(timeout=10) as client:
                r = await client.post(f"{OLLAMA_URL}/api/chat", json={"model": modelo, "messages": msgs, "stream": False, "options": {"temperature": 0.3, "num_predict": 20}})
